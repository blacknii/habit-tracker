...
function DashboardRightHabit(props) {
  ...
  const uncompletedTask = <div className={styles.submit}><button onClick={() => dispatch(completionsSwitch([props.name, lastIndex]))}>Mark Complete</button></div>
  const completedTask = <div className={styles.submit}><p>✅Completed</p><button onClick={() => dispatch(completionsSwitch([props.name, lastIndex]))}>Undo</button></div>
